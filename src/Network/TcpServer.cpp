...
namespace toolkit
{
	...
	/// --------------------------------------------------------------------------------
	/// @brief: TcpServer::recvSpeed
	/// --------------------------------------------------------------------------------
	int TcpServer::recvSpeed()
	{
		if (_socket)
		{
			return _socket->getRecvSpeed();
		}
	...
	/// --------------------------------------------------------------------------------
	/// @brief: TcpServer::sendSpeed
	/// --------------------------------------------------------------------------------
	int TcpServer::sendSpeed()
	{
		if (_socket)
		{
			return _socket->getSendSpeed();
		}
	...
	void TcpServer::onAcceptConnection(const Socket::Ptr& sock)
	{
		...
		session->setOnRecvDataFunc(recv_data_func_);
		session->setTCPErrorFunc(tcp_error_func_);
		session->setTcpFlushFunc(tcp_flush_func_);
		session->setTcpConnectFunc(tcp_connect_func_);

		//把本服务器的配置传递给TcpSession
		session->attachServer(weak_self);
		...
		//会话接收数据事件
		sock->setOnRead([weak_session](const Buffer::Ptr& buf, struct sockaddr* addr, int len)
			{
				//获取会话强应用
				...
				try
				{	
					strong_session->onRecv(buf, addr, len);
				}
